...
pygame.init()
x, y = 1920, 1080  # Largura e altura da janela de simulação
tela = pygame.display.set_mode((x, y))
clock = pygame.time.Clock()
pygame.font.init()
fonte = pygame.font.SysFont('Arial', 16)

# Definição de constantes físicas e características dos corpos celestes
# Utiliza valores aproximados de massa, distância e tamanho dos planetas e do Sol
G = 6.67430e-11  # Constante gravitacional universal
massaSol = 1.989e10  # Massa do Sol em unidades simplificadas
tamanhoSol = 1392.700  # Diâmetro do Sol em milhares de km
distanciaPlanetas = [57.9, 108.2, 149.6, 227.9, 778.5, 1432.0, 2867.0, 4515.0]  # Distância do Sol em milhões de km
massaPlanetas = [3.28e3, 4.83e4, 5.98e4, 6.40e3, 1.90e7, 5.68e6, 8.67e5, 1.05e6]  # Massas dos planetas
tamanhoPlanetas = [4.879, 12.104, 12.756, 6.798, 142.964, 120.536, 51.118, 49.572]  # Diâmetros dos planetas em milhares de km
coresPlanetas = [(169, 169, 169), (255, 215, 0), (0, 191, 255), (188, 39, 50), (255, 140, 0), (210, 180, 140), (0, 255, 255),(25, 25, 112)]  # Cores RGB para cada planeta
nomesPlanetas = ["Mercúrio", "Vênus", "Terra", "Marte", "Júpiter", "Saturno", "Urano", "Netuno"]  # Nomes dos planetas
...
# Define região e quantidade de asteroides entre Marte e Júpiter
ASTEROID_BELT_INNER_RADIUS = 300 + (tamanhoSol/2)  # Raio interno do cinturão em milhões de km
ASTEROID_BELT_OUTER_RADIUS = 600 + (tamanhoSol/2)  # Raio externo do cinturão em milhões de km
NUM_ASTEROIDS = 250  # Número de asteroides a serem gerados

# Variáveis de controle de visualização
zoom = 0.1  # Nível inicial de zoom
pan_x = 0  # Deslocamento horizontal da câmera
pan_y = 0  # Deslocamento vertical da câmera

# Posição inicial do Sol no centro da tela
...
def fGrav(m1, p1, m2, p2): 
    ...
    5. Retorna o vetor força gravitacional (aplica o módulo da força na direção do vetor entre os planetas com 'magnitude * dir')
...
def world_to_screen(pos):
    ...
    Aplica transformações de zoom e deslocamento (pan) para ajustar a visualização
    
    Parâmetros:
        pos (pygame.Vector2): Posição original no espaço de simulação
    
    Retorna:
        pygame.Vector2: Posição ajustada na tela de renderização
    '''
...
def calc_velocidade(objeto):
    ...
    Usa a diferença entre a posição atual e a posição anterior para determinar a velocidade
    
    Parâmetros:
        objeto (dict): Dicionário contendo posição atual e anterior do objeto
...
# Cria uma lista de planetas com posições, velocidades e características iniciais
planetas = []
